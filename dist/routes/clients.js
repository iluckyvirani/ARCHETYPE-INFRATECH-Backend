...
import { addMonths, buildSchedule, calcTotals, mapClientRow, mapScheduleRow, round2, toDateOnly, todayISO, } from "../lib/billing.js";
const router = Router();
function id(prefix) {
...
router.get("/group/:groupId", async (req, res) => {
    ...
    `;
        ...
        const mapped = invoices.map((r) => mapClientRow(r));
      ...
      `;
        ...
        for (const inv of mapped) {
            const hasEmi = merged.some((r) => {
                ...
                const iid = String(r.invoice_id ||
                    r.client_id ||
                    "");
                return (iid === inv.id &&
                    (kind === "installment" || kind === "one_time" || kind === "stage"));
            });
            if (hasEmi || inv.balance <= 0)
                continue;
            const plan = inv.paymentPlan && inv.paymentPlan !== "none"
                ? inv.paymentPlan
                : "one_time";
            ...
            for (const row of rows) {
                ...
                if (row.kind === "advance") {
                    const exists = merged.some((r) => String(r.kind) === "advance" &&
                        String(r.invoice_id ||
                            r.client_id ||
                            "") === inv.id);
                    if (exists)
        ...
        merged.sort((a, b) => String(a.due_date).localeCompare(String(b.due_date)));
        ...
        res.json({
            ...
            invoices: mapped,
            schedule: merged.map((r) => mapScheduleRow(r)),
        });
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Failed";
...
router.post("/group/:groupId/invoices", async (req, res) => {
    ...
    `;
        ...
        const resolvedGroup = existing[0].group_id || String(existing[0].id);
        const body = req.body;
...
router.patch("/invoice/:invoiceId", async (req, res) => {
    ...
    `;
      ...
      `);
            ...
            const alreadyCollected = paidRows.reduce((s, r) => s + (Number(r.paid_amount) || Number(r.amount) || 0), 0);
            const remaining = round2(Math.max(0, balance - alreadyCollected));
            if (unpaidRows.length > 0) {
                const oldUnpaidTotal = unpaidRows.reduce((s, r) => s + (Number(r.amount) || 0), 0);
                let allocated = 0;
          ...
          `;
                ...
                const kind = existing[0].payment_plan === "stage"
                    ? "stage"
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const mapped = mapClientRow(clients[0]);
        const schedule = await sql `
      SELECT * FROM schedule_items
    ...
    `;
        res.json({
            client: mapped,
            schedule: schedule.map((r) => mapScheduleRow(r)),
        });
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Failed";
...
router.patch("/:id/schedule/:scheduleId", async (req, res) => {
    try {
        ...
        const bodyPaidAt = typeof req.body?.paidAt === "string" ? req.body.paidAt.slice(0, 10) : null;
